// 1111134005.12.27.cpp : 此檔案包含 'main' 函式。程式會於該處開始執行及結束執行。
//
...
int main(void)
{
 ...
 {9,77,55,0}, {10,97,68,0} };//用學生id和成績的二維數組
    int i;
    ...
    printf("%10s  %5s %5s %5s %5s\n", "Name", "ID", "Math", "Compu", "Avg");//顯示原始數據

    for (i = 0; i < SIZE; ++i) {
        printf("%10s  %5d %5d %5d %5lf", name[i], my_stu[i].ID, my_stu[i].math, my_stu[i].compu, my_stu[i].avg);
        printf("\n");
    }
    printf("\n");
...
void bubble(struct student my_stu[SIZE])//泡沫排序函數
{
    struct student temp;
    unsigned int pass, i;
    // bubble sort                                         
...
void swap(int* d1ptr, int* d2ptr)//交換函數
{
    int hold = *d1ptr;
    *d1ptr = *d2ptr;
    *d2ptr = hold;
