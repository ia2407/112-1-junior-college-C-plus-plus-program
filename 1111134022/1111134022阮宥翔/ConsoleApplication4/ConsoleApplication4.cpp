...
int main(void)
{
 ...
 {9,77,55,0}, {10,97,68,0} };
    ...
    for (i = 0; i < SIZE; ++i) {
        ...
        student[i].avg = ((double)student[i].math + student[i].computer) / 2;
    }

    printf("%10s  %5s %5s %5s %5s\n", "Name", "ID", "Math", "Computer", "Avg");

    for (i = 0; i < SIZE; ++i) {
        printf("%10s  %5d %5d %5d %5lf", student[i].name, student[i].id, student[i].math, student[i].computer, student[i].avg);
        printf("\n");
    }
    printf("\n");
